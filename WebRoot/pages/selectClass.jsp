  ...
  <head>
	...
	$(function() {
		$('#selectedClass').datagrid({
			url : 'clzz!getSelectableClass.action',
			fit : true,
			title : "已选课程",
			collapsible : true,
			loadMsg : "数据加载中请稍后……",
			...
			toolbar: [{
				iconCls: 'icon-save',
				handler: function(){
					console.info(($('#selectedClass').datagrid('getChecked'))[0].classID);
					var carr = $('#selectedClass').datagrid('getChecked');
					var cs = "";
					for(var i = 0; i< carr.length; i++){
						cs = cs + carr[i].classID + ","
					}
					console.info(cs);
					$.ajax({
						url:"clzz!selectClass.action",
						mehtod:"post",
						data:{classes:cs},
						success:function (){
